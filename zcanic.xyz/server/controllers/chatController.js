...
exports.createSession = async (req, res) => {
  try {
    ...
    const sessionTitle = title || `聊天会话 ${new Date().toLocaleString('zh-CN')}`;
...
exports.getSessionMessages = async (req, res) => {
  try {
    ...
    const [messages] = await pool.query(
      `SELECT id, role, content, status, created_at, task_id 
       FROM chat_messages 
       WHERE session_id = ? 
...
exports.sendMessage = async (req, res) => {
  try {
    ...
    // 4. 保存AI设置到任务中
    const aiSettings = formatAiSettings(model, temperature, maxTokens, systemPrompt);
    ...
    if (Object.keys(aiSettings).length > 0) {
      ...
      logger.info(`[chatController] 已保存AI设置到任务(${taskId})`);
    }
